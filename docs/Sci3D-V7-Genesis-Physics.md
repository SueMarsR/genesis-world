...
### 5.1 已实现批次：`Genesis_VP_Hard`（4 道真实素材难题）

在 §5 PoC（jenga 倒塌，`Genesis_VP_PoC`）之上，基于新生成的**动画 GLB 素材**（机械臂/MPM/FEM/IPC 多 solver）造了一批更难的题，注册为任务集 `Genesis_VP_Hard`，`asset.viewer=genesis_view`（加载动画 GLB，viewer 端 `AnimationMixer` 循环播放物理演化）。挑选标准：**无解析解、必须积分演化才能答**，通过 3D-irreducibility test 最强档。

| # | 标签 | 场景 / GLB | solver | 问题 | GT（从动画末帧实测） | 为何不可解析 |
|---|---|---|---|---|---|---|
| H1 | **V3×P2** | Franka 力控抓取抬升 / `franka_grasp` | SAP（rigid+接触） | 抬升时方块抓稳还是滑落？ | `grasp_holds=true`，上移 0.169 m（质心轨迹 0.02→0.097→0.185→0.189 单调升不回落） | 须积分 SAP 摩擦接触 vs 重力/惯性 |
| H2 | **V6×P2** | 弹性龙撞十字刀 / `cut_dragon` | MPM Elastic + CPIC | 龙被切成几块？ | `n_fragments=4`（末帧 4 个 x-z 象限粒子全有：20351/14740/11483/8132） | 须跑 CPIC 连续体断裂仿真 |
| H3 | **V6×P1** | 沙流喷落交错 4 轮塔 / `sand_wheel` | MPM Sand（颗粒） | 沙最终堆在哪一层？到地吗？ | `bulk=middle`、`reaches_floor=false`（末帧 Y 直方图 {Y0:203, Y1:10266, Y2:1531}，85% 在中层轮） | 须积分颗粒级 MPM 多级偏转级联 |
| H4 | **V2×P2** | 双 FEM 布料落 4×4 方块阵 / `ipc_cloth` | IPC（FEM+rigid 接触） | 布料贴合方块还是架空悬停？ | `drape_conform=true`、`settles_near_cube_height=true`（双布 up 质心 0.10/0.14→0.038/0.050 = 方块顶高 0.05） | 须 FEM 布料 + IPC 接触耦合积分下垂 |

**与 §5 计划表的对应**：H1 实现了计划 #4（夹爪+软/刚耦合，改用 SAP 刚体抓取的更干净 binary）；H2/H3 把 V6（不变量）抬到 P2/P1（断裂后拓扑、颗粒终态），是计划表未列的新交叉；H4 把 V2（遮挡）抬到 P2（形变后构型）。四题覆盖 **V2/V3/V6 × P1/P2** 四个交叉，与 PoC 的 V0×P1 互补。

**关键方法论**：GT **不从脚本臆测**，而是用 stdlib 解析导出 GLB 的 morph 末帧（base POSITION + 最后一个 target）反读真实坐标——保证 GT 与玩家在 viewer 里看到的画面严格一致。每题带 `tool_policy.forbidden_tools`（禁 `scene.step/get_pos/get_particles_pos/...`）enforce「仿真即答案」的 P1/P2 公平性命门。详见 [`Genesis-Physics-VQA-Pipeline.md`](./Genesis-Physics-VQA-Pipeline.md)。

远程访问：`http://10.0.0.132:8000/viewer/sciencevision_tasks.html?set=Genesis_VP_Hard`。
...
| v0.3 | 2026-06-05 | 新增 §5.1：基于真实动画素材实现 `Genesis_VP_Hard` 4 道难题（V2/V3/V6 × P1/P2），GT 从动画末帧实测；配套 [`Genesis-Physics-VQA-Pipeline.md`](./Genesis-Physics-VQA-Pipeline.md)。 |
